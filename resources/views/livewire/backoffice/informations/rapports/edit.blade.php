    ...
    <div class="flex flex-col overflow-hidden rounded-lg bg-white shadow-xs dark:bg-gray-800 dark:text-gray-100">
        ...
        <div class="flex flex-col gap-3 bg-gray-50 px-5 py-4 text-center sm:flex-row sm:items-center sm:justify-between sm:gap-0 sm:text-left dark:bg-gray-700/50">
            <div>
                ...
                <h2 class="text-2xl font-bold">Modifier un rapport</h2>
            </div>
        </div>
        ...
        <div class="flex flex-col mt-4 overflow-hidden rounded-lg bg-white shadow-xs dark:bg-gray-800 dark:text-gray-100">
            <div class="grow p-5 md:flex md:gap-5">
                <form action="{{ route("backoffice.rapports.update", $rapport->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 w-full">
                    @csrf
                    <div class="space-y-1">
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="category_id"
                            ...
                            @foreach ($categories as $categorie)
                                <option value="{{ $categorie->id }}" {{ old('category_id', $rapport->category_id) == $categorie->id ? 'selected' : '' }}>
                                    {{ $categorie->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name='title' value="{{ old("title", $rapport->title) }}" placeholder="Saisir le titre"
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="date" name='publication_date' value="{{ old("publication_date", $rapport->publication_date->format("Y-m-d")) }}"
                            class="block w-full rounded-lg border border-gray-200 px-3 py-2 leading-6 placeholder-gray-500 focus:border-green-500 focus:ring-3 focus:ring-green-500/50 dark:border-gray-600 dark:bg-gray-800 dark:placeholder-gray-400 dark:focus:border-green-500" />
                        <p class="text-sm text-red-600 dark:text-red-400">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name='short_description' value="{{ old("short_description", $rapport->short_description) }}" placeholder="Saisir la description courte"
                            class="block w-full rounded-lg border border-gray-200 px-3 py-2 leading-6 placeholder-gray-500 focus:border-green-500 focus:ring-3 focus:ring-green-500/50 dark:border-gray-600 dark:bg-gray-800 dark:placeholder-gray-400 dark:focus:border-green-500" />
                        <p class="text-sm text-red-600 dark:text-red-400">
                    ...
                    <div class="space-y-1">
                        <label class="group relative inline-flex items-center gap-3">
                            <input type="checkbox" class="peer sr-only" name='is_online' value="{{ old("is_online") }}" {{ $rapport->is_online ? "checked" : "" }}/>
                            <span
                            ...
                            <span class="font-medium">Voulez-vous publier ce rapport ?</span>
                        </label>
                    </div>
                    <button type="submit"
